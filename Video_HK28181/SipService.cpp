...
static unsigned __stdcall jrtplib_rtp_recv_thread(void* arg)
{
	...
	{
		fprintf(g_fp, "fopen %s failed", filename);
		return NULL;
	}
...
static int ParserIniFile(void* ptrHandle, char *iniCfgFile)
{
	...
	if (g_fp == NULL)
		return -1;
	fprintf(g_fp, "开始加载配置文件");

	//liveVideoStreamParams *pHandle = (liveVideoStreamParams*)ptrHandle;
	internal_handle* pHandle = (internal_handle*)ptrHandle;
	::GetPrivateProfileString("YTControl", "platform_id", "你好", pHandle->hliveVideoParams.gb28181Param.platformSipId, MAX_PATH, iniCfgFile);	//获取平台ID
	pHandle->hliveVideoParams.gb28181Param.platformSipPort = GetPrivateProfileInt("YTControl", "platform_port", 0, iniCfgFile);					//获取平台端口
	::GetPrivateProfileString("YTControl", "platform_ip", "你好", pHandle->hliveVideoParams.gb28181Param.platformIpAddr, MAX_PATH, iniCfgFile);	//获取平台IP
	::GetPrivateProfileString("YTControl", "local_id", "你好", pHandle->hliveVideoParams.gb28181Param.localSipId, MAX_PATH, iniCfgFile);		//获取本地ID
	pHandle->hliveVideoParams.gb28181Param.localSipPort = GetPrivateProfileInt("YTControl", "local_port", 0, iniCfgFile);						//获取本地端口
	::GetPrivateProfileString("YTControl", "local_ip", "你好", pHandle->hliveVideoParams.gb28181Param.localIpAddr, MAX_PATH, iniCfgFile);		//获取平台IP
	pHandle->hliveVideoParams.cameraNum = GetPrivateProfileInt("YTControl", "camera_num", 0, iniCfgFile);										//相机数量
	...
	if (pHandle->hliveVideoParams.cameraNum > 0 && pHandle->hliveVideoParams.cameraNum < CAMERA_SUPPORT_MAX) 
	{
		...
		for (int i = 0; i < pHandle->hliveVideoParams.cameraNum; ++i)
		{
			...
			GetPrivateProfileString("YTControl", "camera1_sip_id", "", p->sipId, MAX_PATH, iniCfgFile);
			p->recvPort = GetPrivateProfileInt("YTControl", "camera1_recv_port", 0, iniCfgFile);

			//获取相机登录名和密码
			GetPrivateProfileString("YTControl", "UserPwd", "", p->UserPwd, MAX_PATH, iniCfgFile);
			GetPrivateProfileString("YTControl", "UserName", "", p->UserName, MAX_PATH, iniCfgFile);
		}
	}
...
void  CALLBACK iveInternal_StreamReadyCB(int handle, const char* data, int size, void *pUser)
{
	...
	{
		//fprintf(g_fp, "符合要求的数据%x,,%x,,%x,,%x", buffer[i], buffer[i + 1], buffer[i + 2], buffer[i + 3]);
...
int inline GetH246FromPs(char* buffer, int length, char *h264Buffer, int *h264length, char *sipId)
{
	...
	*h264length = 0;
	...
	while ((unsigned int)leftlength >= sizeof(pack_start_code))
	{
		...
		if (NextPack && NextPack[0] == '\x00' && NextPack[1] == '\x00' && NextPack[2] == '\x01' && NextPack[3] == '\xE0')
		{
			...
			if (Pes(NextPack, leftlength, &NextPack, &leftlength, &PayloadData, &PayloadDataLen))
			{
				if (PayloadDataLen)
				{
					...
					{
						fprintf(g_fp, "h264 frame size exception!! %d:%d", PayloadDataLen, *h264length);
					}
				}
			}
			else
		...
		{
			fprintf(g_fp, "audio ps frame, skip it\n");
			break;
		}
		else
		{
			fprintf(g_fp, "[%s]no know %x %x %x %x\n", sipId, NextPack[0], NextPack[1], NextPack[2], NextPack[3]);
			break;
		}
	}
...
static int stopCameraRealStream(liveVideoStreamParams *pliveVideoParams)
{
	...
	for (i = 0; i < pliveVideoParams->cameraNum; i++)
	{
		...
		if (p28181Params->call_id == -1)
		{
			fprintf(g_fp, "exception wait call_id:%d, %s", p28181Params->call_id, p->sipId);
		}
...
static int checkCameraStatus(liveVideoStreamParams *pliveVideoParams)
{
	...
	for (i = 0; i < pliveVideoParams->cameraNum; i++)
	{
		...
		if (p->status == 0)
		{
			...
			{
				fprintf(g_fp, "camera %s is exception, restart it", p->sipId);
				p28181Params->call_id = -1;
				sendInvitePlay(p->sipId, p->recvPort, p28181Params);
				p->statusErrCnt = 0;
...
static unsigned __stdcall rtp_recv_thread(void *ptrParams)
{
	...
	if (socket_fd >= 0)
	{
		fprintf(g_fp, "start socket port %d success\n", rtp_port);
	}
	...
	fprintf(g_fp, "%s:%d starting ...", ptrHandle->hliveVideoParams.pCameraParams->sipId, ptrHandle->hliveVideoParams.pCameraParams->recvPort);
#endif
	int cnt = 0;
...
#endif 
	{
		...
		if (recvLen > rtpHeadLen)
		{
			...
			{
				fprintf(g_fp, "psBuf memory overflow, %d\n", psLen + rtpPsLen);
				psLen = 0;
				continue;
			}
			//视频流解
			if (ptr[0] == 0x00 && ptr[1] == 0x00 && ptr[2] == 0x01 && ptr[3] == 0xffffffBA&& psLen > 0)
			{
				if (cnt % 10000 == 0)
				{
					fprintf(g_fp, "rtpRecvPort:%d, cnt:%d, pssize:%d\n", rtp_port, cnt++, psLen);
				}
				if (cnt % 25 == 0)
				{
		...
		{
			fprintf(g_fp, "recvfrom() udp");
		}

		if (recvLen > 1500)
		{
			fprintf(g_fp, "udp frame exception, %d\n", recvLen);
		}
	}
	...
	fprintf(g_fp, "%s:%d run over", ptrHandle->hliveVideoParams.pCameraParams->sipId, ptrHandle->hliveVideoParams.pCameraParams->recvPort);
...
static unsigned __stdcall stream_keep_alive_thread(void *arg)
{
	...
	fprintf(g_fp, "%s:%d starting ...", p->sipId, rtcp_port);
	...
	while (p->running)
	{
		...
		if (recvLen > 0)
		{
			fprintf(g_fp, "stream_keep_alive_thread, rtcp_port %d, recv %d bytes\n", rtcp_port, recvLen);
			recvLen = sendto(socket_fd, buf, recvLen, 0, (struct sockaddr*)&servaddr, sizeof(struct sockaddr_in));
			if (recvLen <= 0)
			{
				fprintf(g_fp, "sendto %d failed", rtcp_port);
			}
		}
		else
		{
			fprintf(g_fp, "recvfrom() alive");
		}
	}
	...
	fprintf(g_fp, "%s:%d run over", p->sipId, rtcp_port);
...
static int StartStreamRecv(void* ptrParams)
{
	...
	for (i = 0; i < 1; i++)
	{
...
#endif
		{
			fprintf(g_fp, "pthread_create rtp_recv_thread err, %s:%d", p->sipId, p->recvPort);
		}
		else
		{
			CloseHandle(hHandle);
		...
		if ((hHandleAlive = (HANDLE)_beginthreadex(NULL, 0, stream_keep_alive_thread, (void*)p, 0, NULL)) == INVALID_HANDLE_VALUE) {
			fprintf(g_fp, "pthread_create stream_keep_alive_thread err, %s:%d", p->sipId, p->recvPort + 1);
		}
		else
		{
...
int init_udpsocket(int port, struct sockaddr_in *servaddr, char *mcast_addr)
{
	...
	if (socket_fd < 0)
	{
		fprintf(g_fp, "socket failed, port:%d", port);
		return -1;
	}
	...
	if (err < 0)
	{
		fprintf(g_fp, "bind failed, port:%d", port);
		return -2;
	}
	...
	if (err < 0)
	{
		fprintf(g_fp, "setsockopt IP_MULTICAST_LOOP failed, port:%d", port);
		return -3;
	}
...
static void *MessageThread(SipParams *p28181Params, void * pvSClientGB)
{
	...
	while (p28181Params->running)
	{
		...
		{
			fprintf(g_fp, "recv EXOSIP_CALL_RELEASED\n");
			RegisterSuccess(peCtx, je);
			break;
		}
		case EXOSIP_CALL_CLOSED:
			fprintf(g_fp, "recv EXOSIP_CALL_CLOSED\n");
			RegisterSuccess(peCtx, je);
			break;
		case EXOSIP_CALL_MESSAGE_NEW:
			fprintf(g_fp, "recv EXOSIP_CALL_MESSAGE_NEW\n");
			RegisterSuccess(peCtx, je);
			break;
		default:
			fprintf(g_fp, "##test,%s:%d, unsupport type:%d\n", __FILE__, __LINE__, je->type);
			RegisterSuccess(peCtx, je);
			break;
		}
...
static unsigned __stdcall DaemonThread(void *ptrParams)
{
	...
	{
		fprintf(g_fp, "Can,t initialize, eXosip!");
		return NULL;
	}
	else
	{
		fprintf(g_fp, "eXosip_init successfully!\n");
	}
